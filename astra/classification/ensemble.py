...
class EnsembleClassifier:
    ...
    def combine(
        ...
        result = {
            ...
            "confidence_eb": float(p_final[1]) if len(p_final) > 1 else 0.0,
            "confidence_blend": float(p_final[2]) if len(p_final) > 2 else 0.0,
            "confidence_other": float(p_final[3]) if len(p_final) > 3 else 0.0,
            "tier": tier,
            "proba_xgb": proba_xgb,
            "proba_cnn": proba_cnn,
        }
